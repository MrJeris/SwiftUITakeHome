...
//
//  Created by Ruslan Magomedov on 30.07.2023.
//
...
class JSONMapperTests: XCTestCase {
    ...
    func test_with_missing_file_error_thrown() {
        ...
        do {
            ...
            XCTAssertEqual(mappingError, StaticJSONMapper.MappingError.failedToGetContents, "This should be a failed to get content error")
        }
    }
    
    func test_with_invalid_file_error_thrown() {
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "hlfsfsd", type: UsersResponse.self), "An error should be thrown")
        do {
            _ = try StaticJSONMapper.decode(file: "fjahfkafh", type: UsersResponse.self)
        } catch {
            guard let mappingError = error as? StaticJSONMapper.MappingError else {
                XCTFail("This is the wrong type of error for missing files")
                return
